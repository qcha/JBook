...
> Методы по умолчанию пояивлись в `Java 8` и представляют по сути `default`-ую реализиацию метода интерфейса. При реализации интерфейса мы можем не переопределять такие методы,
