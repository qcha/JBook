...
Но в какой момент вообще происходит инициализаиця класса или интерфейса?
...
> Методы по умолчанию пояивлись в `Java 8` и представляют по сути `default`-ую реализиацию метода интерфейса.
>
> При реализации интерфейса мы можем не переопределять такие методы,
если нас устраивае поведение по-умолчанию, которое реализовано в интерфейсе.

В свою очередь инициализация делится на следующие типы:
### Типы инициализации
...
Это значит, что если вы обладаете всем необходимым для иницаилизации этого
...
Однако, если иницаилизация происходит ,например, вызовом метода или в блоке - то стоит задуматься - возможно, было бы правильно вынести это в конструктор класса?
