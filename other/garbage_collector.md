...
## Эксперементальные GC

`Epsilon GC` является нововведением Java 11. Это эксперементальный сборщик мусора, который очень полезен в целях тестирования приложения. Его суть заключается в отказе от такого понятия, как сборка мусора. Казалось бы, зачем нужен сборщик мусора, который не собирает мусор. Но его появление назревало уже давно.Для того, чтобы понимать, в чем заключается преимущество такого подхода, надо вспомнить, что под собой несёт сама сборка мусора в Java. 

Для начала, надо вспомнить, что непосредственно для сборки необходимы данные, сколько пережил объект, а так же некоторые другие метаданные, которые хранит в своих заголовках объект. Так вот для `Epsilon GC` все эти метаданные не нужны, потому что система вовсе и не собирается обходить граф, искать недостежимые из корня объекты и собираться их, делая `STW` паузу. Таким образом, уже видно преимущество такого подхода.

Само собой, такой подход не подходит для промышленной разработки. Среднестатистическое приложение, а котором исправно работает подходящий ему сборщик мусора, переживает full gc довольно таки часто, и это не считая минорных сборок. Но и такой подход был придуман не просто так, а все же для нужд программистов. Например, одним из плюсов no-op подхода заключается в том, что приложение с таким сборщиком будет полезно сравнить в производительности с приложением с “боевым” gc. Посмотреть какие есть накладные расходы на циклы работы сборщика, насколько увеличивается пауза на сборку, когда есть барьеры на запись/чтение. Все это поможет увидеть, как ваш код работает в первозданном виде. Также,no-op будет полезен для тестирования приложений и фреймворков, которые претендуют на то, что почти не засоряют heap объектами, например, при object-pooling’е. Таким фреймворком, например, является Netty с выделением объектов в offheap область памяти JVM.

Для того, чтобы протестировать свое приложением с использованием `Epsilon GC`, необходимо всего лишь добавть в JVM args два флага: `XX:+UnlockExperimentalVMOptions` `-XX:+UseEpsilonGC`. И все. Дальше остается только ждать, когда ваше приложением выкинет  OutOfMemoryError. Или не выкинет. В целом, каким образом будет происходить остановка приложением - дело самих разработчиков. Можно настроить сборку heap dump’а с помощью флага `-XX:+HeapDumpOnOutOfMemoryError`, либо выполнить какое-то стандартное действие.
Таким образом, Epsilon GC занимает свою нишу в качестве достаточно незаменимого инструмента для тестирования вашего кода. В случае его использования можно увидеть как ваше приложением работает без каких либо накладных расходов, можно провести регрессионной тестирование, померять нагрузку, провести микробенчмарк тесты и все это покажет “чистые” показатели работы вашего приложения.
