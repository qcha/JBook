...
При этом не менее важна не только утечка памяти, но и доступ уже к осовбожденным частям.
...
Данный подход используется во многих языках программирования, например, в `Python`.
...
После остановки мы проходим по всем объектам и помечаем(`mark`) все "живые" объекты, после чего делаем `sweep` - чистим и снимаем все пометки с "живых" объектов после этого.
