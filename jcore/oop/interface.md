...
И да, бывают ситуации, когда мы хотим использовать что-то более конкретное, например, в случае, когда нам нужен порядок добавления элементов в hash-мапу, то необходимо работать с `LinkedHashMap`. И интерфейс `Map` в данном случае не подойдет.
