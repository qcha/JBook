...
Т.е значения этого поля на *данный момент* нет, но в дальнейшем вполне возможно, что оно появится: например, пользователь его добавит в личном кабинете, или укажет в отделении банка сотруднику.
...
Подобный способ довольно распространен: вы пишете метод, а когда нет возвращаемого значения - отдаете `null`.
А теперь представьте, как вы будете пользоваться (и обрекаете на это других) этим методом?
...
Это значит, что поле класса вполне может хранить внутри `null`, проблемы начинаются в момент, когда начинается работа с этим полем - использование его как ссылки.
...
Определенно **не стоит** его использовать в качестве параметров метода, поскольку при использовании `Optional` в качестве параметров метода теряется читабельность, код становится более громоздким, проверять на `null` придется всё равно, и поэтому лучше предоставить действительное значение или сделать перегрузку метода в случаях, когда параметр необязателен.
...
Но как понять тому, кто будет использовать ваш код, какие поля *могут* по нашей задумке быть `null`, а какие - нет, и это *явная* ошибка?
...
Вообще это довольно холиварный вопрос, но если вы пишете на `Java 8`, то `JSR-305` будет к месту.
