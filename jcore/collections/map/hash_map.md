...
![Array example](../../../images/jcore/collections/map/hash_map/array_map_example.png)
...
![Associative array example](../../../images/jcore/collections/map/hash_map/associative_array.png)
...
![buckets](../../../images/jcore/collections/map/hash_map/buckets_example.png)
...
Заметьте, что реализуются интерфейсы-маркеры `java.lang.Cloneable` и `java.io.Serializable`. Т.е структура данных поддерживает [clone](../../object/clone.md) (но помните о подводных камнях такого подхода) и [сериализацию](../../serialization/binary.md).
...
* `final int hash` - hash текущего элемента, который получается в результате хэширования ключа.
...
После добавления будет произведена проверка не превышает ли текущее количество элементов параметр `threshold` и при необходимости делается `resize` для увеличения размера hash-таблицы.
...
![Tree array](../../../images/jcore/collections/map/hash_map/tree_array.png)
...
Здесь также вычисляется `hashCode`, для понимания того, из какой корзины удалять элемент.
