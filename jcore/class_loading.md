...
При этом классы сначала компилируются в байт-код, после чего уже интерпретируются виртуальной машиной `Java`.
...
В JVM по умолчанию используются три основных загрузчика классов:
...
* `Extention Classloader` - загрузчик расширений.
    ...
    Начиная с `Java 9+` он уже не используется.

Существует также возможность создания собственных загрузчиков.

В таком случае ваш собственный загрузчик должен быть унаследован от класса `java.lang.ClassLoader`.

### Иерархия

Загрузчики классов образуют иерархию, корнем которой является базовый загрузчики:
...
И так будет происходить пока либо искомый класс не будет найден у загрузчиков, либо цепочка вызовов не дойдет до корневого загрузчика `Bootstrap`.
...
Такой подход позволяет загружать классы тем загрузчиком, который наиболее приближен к базовому.
...
Для примера приведем следующий код, где тем же самым загрузчиком, что загружал класс `Main`, мы загрузим класс `com.example.Person` (который до этого создали руками сами в IDE):
...
public class Main {
  ...
  public static void main(String[] args) throws Exception {
    ...
    Class cls2 = Class.forName("com.example.Person", true, cLoader);
...
Процесс загрузки класса вкратце выглядит следующим образом:
...
5. Базовый загрузчик также проверяет свой кэш и если не находит класс, то пытается загрузить класс самостоятельно оттуда, областью которой заведует, т.е из `rt.jar`.
...
0. [Как на самом деле работает Java ClassLoader system?](https://habr.com/ru/articles/748758/)
