...
Абстрактный класс `AbstractList` предоставляет заготовку для последующих реализаций.
...
Абстрактный класс `AbstractSequentialList` предоставляет похожую реализацию заготовки, но для списков с последовательным доступом, таким, как `java.util.LinkedList`.
