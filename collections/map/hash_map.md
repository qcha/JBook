...
До этого вы уже были знакомы с массивами, где обращение к элементу просиходило по индексу. Т.е каждому элементу в массиве соответствует свой номер.
...
> Подобный способ разрешения ситуации с коллизиями элементов называется [chaning](https://ru.wikipedia.org/wiki/%D0%A5%D0%B5%D1%88-%D1%82%D0%B0%D0%B1%D0%BB%D0%B8%D1%86%D0%B0#%D0%9C%D0%B5%D1%82%D0%BE%D0%B4_%D1%86%D0%B5%D0%BF%D0%BE%D1%87%D0%B5%D0%BA). Существует еще также и [метод открытой адресации](https://ru.wikipedia.org/wiki/%D0%A5%D0%B5%D1%88-%D1%82%D0%B0%D0%B1%D0%BB%D0%B8%D1%86%D0%B0#%D0%9E%D1%82%D0%BA%D1%80%D1%8B%D1%82%D0%B0%D1%8F_%D0%B0%D0%B4%D1%80%D0%B5%D1%81%D0%B0%D1%86%D0%B8%D1%8F), однако в `java.util.HashMap` за реализацию взят именно `chaning`.
...
Важно понимать, что `size` - это не размер ассоциативного массива, это именно количество хранящихся элементов.
...
И тут возникает вопрос: что такое `Node` и зачем он нужен? 

**Ответ**:

В случае полного отсутствия коллизий хватило бы простого массива, так как каждый элемент находился бы в своей ячейке. Но коллизии неизбежны, даже при очень хорошей хэш-функции они возможны. А при коллизии два элемента попадали бы в одну ячейку. Именно поэтому `table` - это не просто массив, а массив корзин, **buckets**.
...
Структура данных `java.util.HashMap` имеет встроенные итераторы. Получить можно как список всех ключей `keySet()` или всех значений `values()`, так и все пары ключ-значение `entrySet()`.
