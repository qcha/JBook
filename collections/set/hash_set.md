...
Стоит отметить, что `capacity` меньше `0` выставить не получится, так как выбросится исключение `IllegalArgumentException`, что довольно логично.
...
Благодаря тому, что в основе реализации лежит `java.util.HashMap`, реализация `java.util.HashSet` предоставляет для операций добавления, удаления и проверки на содержание элемента константное время: `O(1)`.
...
Подробнее про [java.util.HashMap](../map/hash_map.md).
