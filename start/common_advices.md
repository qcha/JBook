...
В `Java 8+` появились [специальные классы](https://docs.oracle.com/javase/8/docs/api/java/util/Optional.html), призванные помочь с этой проблемой - возвращения `null` при отсутствии значения, хотя и это не панацея.
...
Для ответа на вопрос, что закрывать, а что нет смотрите на класс, который вы пишите и задавайте вопрос - является ли этот метод частью открытого интерфейса? Класс `EventPusher` должен предоставлять возможность отправки событий, как следует из его названия. Значит `pushEvent` - это часть открытого интерфейса. Должен ли этот класс уметь создавать **вам** новый продюсер? Наврятли, этого никто не ждет от него. Но этот метод нужен для инициализации продюсера, занимающегося отправкой событий - значит это закрываем от доступа извне.
...
Важным правилом, которое надо стараться соблюдать, является то, что в коммите, решающим определенную задачу, не должно быть никаких изменний кода, не связанных с задачей.
...
Представим ситуацию, когда вам надо обновить версию библиотеки в проекте. По ходу выполнения задачи вы делаете еще небольшой рефакторинг. Но позже оказывается, что обновление библиотеки было зря, например, новая версия оказалась сырой и эти изменения по обновлению надо откатить. И в этом случае мы теряем и код-рефакторинг, ведь у нас все в одном коммите!
И по сути работа по рефактроингу проделана зря.

Лучше разбейте на два коммита такие изменения. В таком случае коммит с рефактроингом мы не потеряем, при необходимости его можно даже `cherry pick`-нуть в другую ветку.
