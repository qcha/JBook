...
В целом, подобный код часто встречается и это не так уж плохо: у нас есть обязательные значения(`name` и `age`) и необязательные, которые **могут** отсутствовать - электронный адрес.
