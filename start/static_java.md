...
При этом, все эти изменения затрагивают **только** тот массив, с которым мы работаем и никак не влияют на другие.
...
Например, мы хотим написать метод, который печатает в консоль число, которое передается аргументом в этот метод: 
...
Если мы напишем такой код, то для каждого использования нашего метода необходим объект нашего класса.
При этом мы видим, что объект нам по сути то и не нужен, так как все что нам надо для работы мы передаем в аргументах метода, т.е **контекст** выполняемой операции полностью доступен из аргументов метода и результат не влияет на состояние объекта.

Т.е такой метод не связан с **объектом** класса, он связан только с классом, в котором объявлен.

А раз он не связан с объектом, то нам по идее и не нужен объект, для использования этого метода.
Нам нужен только класс, в котором объявлен этот метод, для его использования.
...
Из того, что `static`-члены принадлежат именно **классу** следует еще и то, что мы не можем получить доступ к нестатическим членам, т.е к тому, что принадлежит объекту.
...
Это довольно логично, так как экземпляра объекта класса может еще не быть, поэтому у нас нет переменной `count`.
...
Чаще всего можно задуматься, чтобы сделать метод статическим только в том случае, если весь контекст для его работы передается в аргументах.
...
Очень часто статические методы, принадлежащие к одной области работы, группируются в один класс.
Такие классы обычно имеют в своем имени слова типа `Utils`, тем самым намекая на то, что это класс утилит.
...
Т.е такие классы - это наборы вспомогательных утилит.
...
Про константы и их оформление я написал [вот тут](classes_for_static.md).
...
Мы можем инициализацию вынести в конструктор, но тогда при каждом создании экземпляра объекта класса будет происходить заполнение каталога, а нам надо, чтобы каталог был заполнен только один раз.
...
Данный пример непрозрачно намекает нам, что использовать статические методы при наследовании **не стоит**.
