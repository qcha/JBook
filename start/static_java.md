...
Если говорить совсем простым языком, то можно сказать, что класс - это как чертеж некоторой детали,
т.е то какой она должна быть: материал, форма, из чего состоять должны и т.д.


А объект - это уже сама деталь, со своим уникальным набором свойств.
В примере про деталь - у детали может быть цвет,
материал изготовления, форма, она может состоять также из более мелких деталей и т.д

Это все - свойства объекта, они уникальны - для каждой детали они свои.
...
И применительно к переменным и методам классов - `static` именно так и сработает.
...
И изменение этого поля у одного объекта - это изменение его у всех объектов.
...
Наиболее часто встречаеме применение - это первые два пункта - поля и методы.
...
Т.е контекст операции - это число. Оно передается в аргументах и все - можно уже работать.


Если же для работы статического метода передаваемых ему
аргументов недостаточно - это верный признак к тому, чтобы крепко задуматься -
...
Хороший пример этому - `FileUtils`, который есть в большом количестве библиотек, например - `Apache Commons IO`.
...
Еще часто можно встретить использование статических методов для
создания экземпляров объектов в зависимости от каких то условий,
добавления промежуточной логики в создание объекта и т.д

Примеров такого использования много, они есть и в стандартной библиотеке, например, это `String.valueOf(15)`, `Integer.valueOf("14")` и т.д
...
Все дело именно в том, что при использовании статических методов
...
кроме чисел с плавающей точкой, имеют кеш. По умолчанию данный кеш содержит значения
 от `-128` до `127`, если значение попадает в кеш - оно достается в разы быстрее,
...
```
И тут, я думаю, все понятно - константа - она константа и есть.
...
Благодаря этому статические блоки кода используются тогда, когда необходимо выполнить какую-то логику еще до создания экзмепляра объекта.

Грубо говоря - это конструктор для всего класса.
...
Т.е у нас есть некоторый каталог и есть данные для заполнения. Мы можем инициализацию вынести в конструктор, но тогда при каждом создании экземпляра объекта класса будет происходить заполнение каталог, а нам надо, чтобы каталог был заполнен только один раз. Для этого мы и использовали статический блок кода - и он выполнился ровно один раз - при иницализации класса.
...
> Однако помните!
> Статичный блок **НЕ** может пробросить перехваченные исключения, но может выбросить не перехваченные. В случае возникновения исключения в статическом блоке кода выбросится `ExceptionInInitializerError`.
...
Вам нужен класс - `Node` - в котором вы будете хранить значение, а также ссылки на предыдущий и следующий элемент списка.
Этот класс будет использоваться **только** внутри вашей реализации - и нигде больше он не нужен.
Логично, что раз так - то делать отдельный `public` класс - излишне.


Объявив же в такой ситуации вложенный класс вы добьетесь сразу нескольких вещей:
* Логически сгрупируете классы, которые используются в одном месте

  Если класс полезен только для одного другого класса, то логично вставить его в этот класс и поддерживать вместе.

* Это способ привести код к более читабельному и поддерживаемому виду.

  Благодаря тому, что вы можете объявить вложенный класс ближе к месту использования и скрыть его от использования в других классах.
* Это увеличивает инкапсуляцию.

  Если вложенный класс используется **только** внутри другого класса, то логично скрыть его и использовать только внутри. Аналогично с тем, как мы объявляем `private` поля.
...
Пусь у нас есть некоторый класс со статическим полем:
```java
class Example {
...
Есть два варианта:
* Создать экземпляр объекта класса и обратиться к полю через него
...
Мы не увидим `NullPointerException`, так как вместо `ex` в месте обращения к статической переменной `field` компилятор подставит `Example` - имя класса. Ведь мы обращаемся к полю, которое принадлежит классу! А значит обращение будет не к объекту экземпляра и код, который выполнится будет:
```java
...
Запомните: обращение к статическим методам и полям - через *имя* класса.
...
При таком подходе вы легко можете допустить ошибку и не заметить ее - так как все это компилируется, так что если вы не хотите потерять друзей, то избегайте подобного подхода.
