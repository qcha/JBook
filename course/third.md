...
Коллекции в Java, они же collections - это динамические структуры данных для хранения объектов. Для того, чтобы понять в чем различие между обычным массивом и колллекциями, надо вспомнить какие ограничения задаются обычным массивом.
...
вам, как разработчику, нет необходимости заботиться о том, какого размера у вас массив, сколько объектов у вас сейчас в нем находится, и бояться получить ошибку переполнения массива при добавлении нового элемента.

Иерархия существующих коллекций Java выглядит следующим образом:

<img src="../images/collections/collections.png">

Как видно на картинке выше, в Java есть несколько реализаций коллекций. Каждая из них выполняет свою функцию и отвечает разным требованиям, которые ставятся перед разработчиком.
