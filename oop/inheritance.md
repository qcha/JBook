...
Для нас очевидно, что эти классы связаны логически, т.е `Employee` - является это некоторый `Person` устроенный на работу, но для `Java` эта связь совсем не очевидна, с точки зрения `Java` это просто два разных, никак не связанных и не имеющих ничего общего класса.
...
>
> Про модификаторы доступа нужно почитать [тут](./access_modifiers.md).
...
Наследование - это очень мощный инструментб позволяющий выстраивать иерархии классов, использовать переиспользование кода и бороться с дублированием.
