...
Однако, бывают ситуации, когда мы можем описать класс, но определение его поведения **не завершено**.
Т.е это что-то абстрактное, не до конца определенное, не конкретное.
...
При этом `Person` имеет некоторые свойства: имя, возраст, рост и т.д.
`Person` имеет и некоторое поведение: ходить, спать и т.д.

Однако он не имеет главного - он не конкретен, он абстрактен, при этом он обладает и `поведением`, и `состоянием`.
...
Это ещё не законченный чертёж, строить дом по нему нельзя, но на его основе уже можно делать разные варианты законченных чертежей, которые уже можно использовать в строительстве разных домов.
...
В качестве примера использования абстрактных классов можно посмотреть реализацию `java.util.List`.
