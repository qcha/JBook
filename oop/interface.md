...
Сделаем дефолтную реализацию `hello` у `Two`:
...
Как вы думаете, что будет, если мы реализуем интерфейсы `One` и `Two` у класса?

Правильно, такой код будет не валиден и `Java` вам явно скажет, что так нельзя, так как каждый из интерфейсов тянет за собой какую-то дефолтную реализацию методов.
